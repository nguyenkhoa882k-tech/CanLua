...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Modal, FlatList, Alert, StyleSheet, Dimensions } from 'react-native';
import SimpleDatePicker from '../components/SimpleDatePicker';
import { PieChart } from 'react-native-chart-kit';
import { listTransactions, createTransaction, updateTransaction, deleteTransaction, INCOME_TYPES, EXPENSE_TYPES } from '../services/transactions';
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {incomePieData.length > 0 && (
          <View className="mx-5 mb-4 bg-white rounded-2xl p-4" style={styles.shadow}>
            <Text className="text-lg font-bold text-gray-800 mb-3">📊 Thu theo loại</Text>
            <PieChart
              data={incomePieData}
        ...
        {expensePieData.length > 0 && (
          <View className="mx-5 mb-4 bg-white rounded-2xl p-4" style={styles.shadow}>
            <Text className="text-lg font-bold text-gray-800 mb-3">📊 Chi theo loại</Text>
            <PieChart
              data={expensePieData}
              width={SCREEN_WIDTH - 70}
              height={200}
        ...
        <View className="flex-row px-5 mb-4" style={{ gap: 8 }}>
          <TouchableOpacity
            ...
            className={`flex-1 py-3 rounded-xl ${filter === 'all' ? 'bg-emerald-500' : 'bg-gray-200'}`}
          >
            <Text className={`text-center font-bold ${filter === 'all' ? 'text-white' : 'text-gray-600'}`}>Tất cả</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setFilter('income')}
            className={`flex-1 py-3 rounded-xl ${filter === 'income' ? 'bg-green-500' : 'bg-gray-200'}`}
          >
            <Text className={`text-center font-bold ${filter === 'income' ? 'text-white' : 'text-gray-600'}`}>Thu</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setFilter('expense')}
            className={`flex-1 py-3 rounded-xl ${filter === 'expense' ? 'bg-red-500' : 'bg-gray-200'}`}
          >
            <Text className={`text-center font-bold ${filter === 'expense' ? 'text-white' : 'text-gray-600'}`}>Chi</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="px-5">
          {filteredTransactions.map(transaction => (
            <View key={transaction.id} className="bg-white rounded-2xl p-4 mb-3" style={styles.shadow}>
              <View className="flex-row items-start justify-between mb-2">
                <View className="flex-1">
                  <View className="flex-row items-center mb-1">
                    <Text className="text-xl mr-2">{transaction.type === 'income' ? '📈' : '📉'}</Text>
                    <Text className="font-bold text-gray-800 flex-1">{transaction.category}</Text>
                  </View>
                  <Text className="text-gray-500 text-xs">📅 {new Date(transaction.date).toLocaleDateString('vi-VN')}</Text>
              ...
              <View className="flex-row" style={{ gap: 8 }}>
                ...
                >
                  <Text className="text-blue-600 text-center font-semibold">✏️ Sửa</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => handleDelete(transaction.id)}
                  className="flex-1 bg-red-50 py-2 rounded-xl"
                >
                  <Text className="text-red-600 text-center font-semibold">🗑️ Xóa</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))}
          ...
          {filteredTransactions.length === 0 && (
            <View className="items-center py-10">
              ...
              <Text className="text-gray-400 text-base">Chưa có giao dịch nào</Text>
            </View>
          )}
        </View>
      </ScrollView>
          ...
          <View className="bg-white rounded-3xl p-6 w-full max-w-md">
            ...
            <View className="flex-row mb-4" style={{ gap: 8 }}>
              <TouchableOpacity
                ...
                className={`flex-1 py-3 rounded-xl ${type === 'income' ? 'bg-green-500' : 'bg-gray-200'}`}
              >
                <Text className={`text-center font-bold ${type === 'income' ? 'text-white' : 'text-gray-600'}`}>📈 Thu</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => { setType('expense'); setCategory(''); }}
                className={`flex-1 py-3 rounded-xl ${type === 'expense' ? 'bg-red-500' : 'bg-gray-200'}`}
              >
                <Text className={`text-center font-bold ${type === 'expense' ? 'text-white' : 'text-gray-600'}`}>📉 Chi</Text>
              </TouchableOpacity>
            </View>

            {/* Category */}
            <Text className="text-gray-700 font-semibold mb-2">Loại {type === 'income' ? 'thu' : 'chi'}</Text>
              ...
              <View className="flex-row" style={{ gap: 8 }}>
                {(type === 'income' ? INCOME_TYPES : EXPENSE_TYPES).map(cat => (
                  <TouchableOpacity
                    ...
                    className={`px-4 py-2 rounded-xl ${category === cat ? 'bg-emerald-500' : 'bg-gray-100'}`}
                  >
            ...
            {/* Amount */}
            <Text className="text-gray-700 font-semibold mb-2">Số tiền (đ)</Text>
            <MoneyInput
